...
export function SessionMetadata({ session }: SessionMetadataProps) {
  const rows: Row[] = [
    ...
    { k: 'Last Active',  kind: 'text', v: formatDate(session.lastActiveAt, true) },
    { k: 'Path',         kind: 'copy', v: session.projectPath,                max: 60, expandable: true },
    { k: 'Last Prompt',  kind: 'copy', v: session.lastPrompt ?? '—',          max: 80, expandable: true },
  ];
